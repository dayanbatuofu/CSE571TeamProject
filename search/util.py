...
class PQueue:
    ...
    def insert(self, u, calculateKey):
        ...
        if u in self.UMapToK1:
            self.removeU(u)  # update the value of u if it already exists
        ...
        self._size += 1
        if self.minKey is None or k1 < self.minKey:  # top key identification
            self.minKey = k1
            self.minCount = 1
    ...
    def topKey(self):
        ...
        u_kTopList = []
        for u in uList:
            k1, k2 = self.UMapToK1[u] 
            u_kTopList.append((u, k1, k2))
        u_kTopList.sort(key=lambda x: x[2])
        # print(u_kTopList)
        u_kTop = u_kTopList[0]
